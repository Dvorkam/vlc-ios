...
struct SimpleEntry: TimelineEntry {
    ...
    var decodedData: MLWidgetBridge? = {
        let encodedData = UserDefaults(suiteName: "group..vlc-ios")!.object(forKey: "media") as? Data
...
extension ConfigurationAppIntent {
    fileprivate static var continuePlayback: ConfigurationAppIntent {
        let intent = ConfigurationAppIntent()
//        intent.buttonTitle = "Continue"
        return intent
    }

    fileprivate static var resumePlayback: ConfigurationAppIntent {
        let intent = ConfigurationAppIntent()
//        intent.buttonTitle = "Resume"
        return intent
    }
}
...
#Preview(as: .systemMedium) {
    ...
    SimpleEntry(date: .now, configuration: .continuePlayback)
    SimpleEntry(date: .now, configuration: .resumePlayback)
}
